    ...
    public static ClassScan scan(byte[] classBytes, String archive, Predicate<String> ownerFilter) {
        ...
        for (MethodNode method : node.methods) {
            ...
            for (AbstractInsnNode insn : method.instructions) {
                // An if-chain rather than a pattern-matching switch: this module targets Java 17
                // so the plugin can run on servers that have not moved past it.
                if (insn instanceof MethodInsnNode) {
                    ...
                    noteInternals(internals, ((TypeInsnNode) insn).desc, ownerFilter);
                    noteApiType(apiTypes, ((TypeInsnNode) insn).desc, ownerFilter);
                } else if (insn instanceof InvokeDynamicInsnNode) {
                    collectHandles(references, internals, (InvokeDynamicInsnNode) insn, site, ownerFilter);
                } else if (insn instanceof LdcInsnNode) {
                    Object constant = ((LdcInsnNode) insn).cst;
                    if (constant instanceof String) {
                        constants.add(new ClassScan.StringConstant((String) constant, site));
                    } else if (constant instanceof Type) {
                        noteInternalsInDescriptor(internals, ((Type) constant).getDescriptor(), ownerFilter);
                    }
                }
            }
        }
